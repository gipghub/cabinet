// Bottle.jsx — SVG pill bottle. Sized for shelf display + detail hero.
...
function Bottle({
  ...
  dose = '500mg',
  width = 64,
  variant = 'shelf', // 'shelf' | 'hero'
  fillRatio = 1,
}) {
  const p = window.Med.presets[preset] || window.Med.presets.amber;
  // Bottle proportions: 64 wide × 96 tall typical
  ...
  // Liquid/pill fill area at bottom of bottle (subtle)
  const fillH = bodyH * 0.18 * fillRatio;
  const fillY = bodyY + bodyH - fillH - 2;

  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ display: 'block', filter: 'drop-shadow(0 6px 6px rgba(0,0,0,0.18))' }}>
      <defs>
      ...
      {variant === 'shelf' && (
        <>
          <text x={w/2} y={labelY + labelH*0.34}
                textAnchor="middle"
                fontFamily="Instrument Serif, Georgia, serif"
                fontSize={Math.max(w*0.16, 9)}
                fill={p.accent}
                letterSpacing="0.3">
          ...
          <text x={w/2} y={labelY + labelH*0.7}
                textAnchor="middle"
                fontFamily="Geist, system-ui, sans-serif"
                fontSize={Math.max(w*0.085, 6)}
                fill={p.accent}
                opacity="0.7"
                letterSpacing="0.6">
            {dose}
      ...
      {variant === 'hero' && (
        <>
          <text x={w/2} y={labelY + labelH*0.28}
                textAnchor="middle"
                fontFamily="Instrument Serif, Georgia, serif"
                fontSize={w*0.13}
                fill={p.accent}>{name}</text>
          <text x={w/2} y={labelY + labelH*0.42}
                textAnchor="middle"
                fontFamily="Geist, system-ui, sans-serif"
                fontSize={w*0.06}
                fill={p.accent} opacity="0.7"
                letterSpacing="2">{(sub || '').toUpperCase()}</text>
          ...
          <text x={w/2} y={labelY + labelH*0.66}
                textAnchor="middle"
                fontFamily="Instrument Serif, Georgia, serif"
                fontSize={w*0.1}
                fill={p.accent}>{dose}</text>
          <text x={w/2} y={labelY + labelH*0.83}
                textAnchor="middle"
                fontFamily="Geist, system-ui, sans-serif"
                fontSize={w*0.055}
                fill={p.accent} opacity="0.55"
                letterSpacing="1.5">OVER-THE-COUNTER</text>
        </>
      )}
    </svg>
  );
